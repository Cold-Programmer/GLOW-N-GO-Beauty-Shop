...
exports.confirmByToken = confirmByToken;
...
async function createAppointment(req, res) {
    ...
    if (stylistId) {
        const clash = await prisma_1.prisma.appointment.findFirst({
...
async function listAssignedAppointments(req, res) {
    if (!req.user)
        return res.status(401).json({ success: false, message: "Authentication required." });
    const appointments = await prisma_1.prisma.appointment.findMany({
...
async function updateAppointmentStatus(req, res) {
    ...
    const appointment = await prisma_1.prisma.appointment.update({ where: { id: req.params.id }, data: { status } });
...
async function cancelAppointment(req, res) {
    ...
    const appointment = await prisma_1.prisma.appointment.update({ where: { id: req.params.id }, data: { status: "CANCELLED" } });
    res.json({ success: true, appointment });
}
/**
 ...
 */
async function getAppointmentByToken(req, res) {
...
async function confirmByToken(req, res) {
    const appointment = await prisma_1.prisma.appointment.findUnique({ where: { qrToken: req.params.qrToken } });
    if (!appointment) {
        return res.status(404).json({ success: false, message: "This appointment QR code is invalid or has expired." });
    }
