    ...
    return (<div className="mx-auto max-w-6xl px-5 py-16">
      ...
      <div className="mt-8 flex flex-wrap items-center gap-3">
        <div className="flex flex-wrap gap-2">
          {GENDERS.map((g) => (<button key={g} onClick={() => setGender(g)} className={`rounded-full border px-4 py-1.5 text-xs font-medium capitalize transition-colors ${gender === g ? "border-rosegold-dark bg-rosegold-dark/10 text-rosegold-dark" : "border-ink/15 text-ink/60 hover:border-rosegold"}`}>
              {g === "ALL" ? "All" : g.charAt(0) + g.slice(1).toLowerCase()}
            </button>))}
        </div>
        <select value={category} onChange={(e) => setCategory(e.target.value)} className="ml-auto rounded-full border border-ink/15 px-4 py-1.5 text-xs focus:border-rosegold focus:outline-none">
      ...
      {!loading && !error && products.length === 0 && (<p className="mt-10 text-center text-ink/50">No products match these filters yet.</p>)}

      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {products.map((p) => (<div key={p.id} className="card overflow-hidden bg-white">
            <div className="relative h-48 w-full">
              <SafeImage src={p.image} alt={p.name} fill className="object-cover"/>
              {p.compareAtPrice && (<span className="absolute left-3 top-3 rounded-full bg-rosegold-dark px-2.5 py-1 text-[11px] font-semibold text-white">
                  Sale
                </span>)}
            </div>
            <div className="p-5">
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                {p.compareAtPrice && (<span className="text-xs text-ink/40 line-through">KES {p.compareAtPrice.toLocaleString()}</span>)}
              </div>
