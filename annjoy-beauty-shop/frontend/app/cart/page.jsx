...
export default function CartPage() {
    ...
    if (lines.length === 0) {
        return (<div className="mx-auto max-w-2xl px-5 py-24 text-center">
    ...
    return (<div className="mx-auto max-w-4xl px-5 py-16">
      ...
      <div className="mt-8 divide-y divide-ink/10 rounded-xl2 border border-ink/10 bg-white">
        {lines.map((l) => (<div key={l.product.slug} className="flex items-center gap-4 p-4">
            <div className="relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-lg">
              <SafeImage src={l.product.image} alt={l.product.name} fill className="object-cover"/>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => updateQuantity(l.product.slug, l.quantity - 1)} className="h-8 w-8 rounded-full border border-ink/15 text-sm hover:border-rosegold" aria-label={`Decrease quantity of ${l.product.name}`}>
                −
              </button>
              <span className="w-6 text-center text-sm">{l.quantity}</span>
              <button onClick={() => updateQuantity(l.product.slug, l.quantity + 1)} className="h-8 w-8 rounded-full border border-ink/15 text-sm hover:border-rosegold" aria-label={`Increase quantity of ${l.product.name}`}>
                +
              </button>
            </div>
            <p className="w-24 text-right font-medium">KES {(l.product.price * l.quantity).toLocaleString()}</p>
            <button onClick={() => removeItem(l.product.slug)} className="ml-2 text-xs text-ink/40 hover:text-rosegold-dark" aria-label={`Remove ${l.product.name}`}>
              Remove
            </button>
          </div>))}
      </div>
