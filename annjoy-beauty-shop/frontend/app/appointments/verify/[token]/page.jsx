...
export default function VerifyAppointmentPage() {
    ...
    return (<div className="mx-auto max-w-md px-5 py-16">
      ...
      {error && (<div className="mt-8 rounded-xl2 border border-red-200 bg-red-50 p-6 text-center text-red-700">
          {error}
        </div>)}

      {appointment && (<div className="card mt-8 p-6">
          ...
          {appointment.stylist && (<p className="text-sm"><span className="text-ink/50">Stylist:</span> {appointment.stylist.firstName} {appointment.stylist.lastName}</p>)}
          <p className="mt-3 text-sm">
            <span className="text-ink/50">Status:</span>{" "}
            <span className="font-medium">{confirmed ? "CONFIRMED" : appointment.status}</span>
          </p>

          {!confirmed && (<button onClick={handleConfirm} disabled={confirming} className="btn-primary mt-6 w-full">
              {confirming ? "Confirming…" : "Confirm check-in"}
            </button>)}
          {confirmed && (<div className="mt-6 rounded-lg bg-green-50 p-4 text-center text-sm font-medium text-green-700">
              Checked in ✓
            </div>)}
