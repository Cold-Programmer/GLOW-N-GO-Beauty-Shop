...
export default function Footer() {
    return (<footer className="border-t border-ink/10 bg-ink text-cream/80">
      <div className="mx-auto grid max-w-6xl gap-10 px-5 py-14 md:grid-cols-4">
        ...
        ].map((s) => (<a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="rounded-full border border-cream/20 px-3 py-1.5 text-xs transition-colors hover:border-gold hover:text-gold">
                {s.label}
              </a>))}
          </div>
        </div>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm">
            {businessInfo.hours.map((h) => (<li key={h.day} className="flex justify-between gap-4">
                <span>{h.day}</span>
