...
export default function CheckoutPage() {
  ...
  return (
    <div className="mx-auto max-w-xl px-5 py-16">
      ...
      <form onSubmit={handlePay} className="mt-8 space-y-5">
        <div>
          ...
          <input
            ...
            placeholder="e.g. Ndagani shopping centre, near..."
        ...
        {message && (
          <p role="status" className={`text-sm ${state === "failed" ? "text-red-600" : "text-ink/70"}`}>
            {message}
          </p>
        )}
      </form>
      ...
      {showConfirmed && (
        <PaymentConfirmedModal receiptNo={receiptNo} onClose={() => setShowConfirmed(false)} />
      )}
    </div>
  );
}
