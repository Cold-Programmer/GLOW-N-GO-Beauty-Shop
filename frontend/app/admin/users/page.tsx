...
interface AdminUser {
  ...
  isOnline: boolean;
  _count: { orders: number; appointments: number; activityLogs: number };
}
...
export default function AdminUsersPage() {
  ...
  async function changeStatus(id: string, status: string) {
    if (status !== "ACTIVE" && !confirm(`Set this account's status to ${status}? This takes effect immediately.`)) return;
    setBusyId(id);
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-16">
      ...
      {!loading && !error && (
        <div className="mt-8 overflow-x-auto rounded-xl2 border border-ink/10 bg-white">
