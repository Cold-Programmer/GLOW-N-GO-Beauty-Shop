  ...
  useEffect(() => {
    fetch(`${API}/api/services`, { credentials: "include" })
      .then(async (res) => {
        ...
        const preselected = data.items?.find((s: ApiService) => s.slug === preselectedSlug);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${API}/api/appointments`, {
        ...
        credentials: "include",
        body: JSON.stringify({ serviceId, date, timeSlot: time, notes }),
      });
      const data = await res.json();
      if (!res.ok) {
      ...
      <form onSubmit={handleSubmit} className="mt-10 space-y-5">
        <div>
          ...
          >
            ...
            {apiServices.map((s) => (
              <option key={s.id} value={s.id}>{s.name} — KES {s.price.toLocaleString()}</option>
            ))}
          </select>
        </div>
